...
public class Test01_1 {
  ...
  public static void main(String[] args) throws Exception {
    ...
    // ..을 잘못 사용하면 예외 처리를 한다
    //=>  그래서 이 메서드를 호출 할 때는 던졌을 때 받을 수 있도록 예외처리 문자을 작성해야 한다
    System.out.printf("getCanonicalPath = %s\n", f.getCanonicalPath());
    
    //getFreeSpace()
    //=> File의 경로가 가리킨큰 디스크의 여유 공간을 바이트 단위로 리턴한다.
    ...
    //getPath()
    //=>FILE 크래스에서 지정한 디렉토리 또는 파일의 경로
    System.out.printf("getPath() = %s\n", f.getPath());
    
    //exists()
    //=>해당 경로의 디렉토리 또느 파일의 존재 여부를 리턴한다
    System.out.printf("exists() = %s\n", f.exists());
    
    //isDirectory()
    //=>File 객체가 가리키는 경로가 파일 디렉토리인지 리턴한다.
