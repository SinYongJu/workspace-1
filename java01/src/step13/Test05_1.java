 ...
 * => 클래스를 정의할 떄 이름이 없다.
 * => 이름이 없기 떄문에 클래스를 정의한 후 나중에 따라 인스턴스를 생성할 수 없다.
 * => 클래스를 정의하자 마자 바로 인스턴스를 생성해야 한다.
 * => 보통 클래스를 딱 한 번만 정의해서 사용할 경우에 적합한 문법이다.
 * => 익명클래스 만드는 문법
 *  1) 참조변수 선언 = new 수퍼클래스(또는 인터페이스 이름)(생성자 호출파라미터){...}
 *  2) 참조변수 선언 = new 인터페이스(또는 인터페이스 이름)(생성자 호출파라미터){...}
 */
